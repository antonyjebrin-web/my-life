...
export default function FloatingButtons() {
  ...
  return (
    <div className="fixed bottom-5 right-4 z-40 flex flex-col items-end gap-3 sm:right-6">
          ...
          >
            <div className="flex items-center justify-between bg-[#25D366] px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-white">{t.floating.whatsappHeader}</p>
                  <p className="text-xs text-white/80">{t.floating.whatsappSubtext}</p>
                </div>
              </div>
              <button
            ...
            <div className="p-3">
              <div className="mb-2 rounded-lg bg-gray-100 p-3 dark:bg-slate-700">
                <p className="text-sm text-gray-700 dark:text-gray-200">👋 {t.floating.whatsappSubtext}</p>
              </div>
      ...
      <motion.button
        onClick={() => {
          const encodedMessage = encodeURIComponent(t.floating.whatsappSubtext);
          window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodedMessage}`, '_blank');
        }}
